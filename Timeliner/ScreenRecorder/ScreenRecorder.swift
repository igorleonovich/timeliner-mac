...
public final class ScreenRecorder: NSObject {
  ...
  public init(
    ...
    let input = try AVCaptureScreenInput(displayID: screenId).unwrapOrThrow(ScreenRecorderError.invalidScreen)
    ...
    if let audioDevice = audioDevice {
      if !audioDevice.hasMediaType(.audio) {
        throw ScreenRecorderError.invalidAudioDevice
      }
      ...
      if session.canAddInput(audioInput) {
        ...
        throw ScreenRecorderError.couldNotAddMic
      }
    }
    ...
    if session.canAddInput(input) {
      ...
      throw ScreenRecorderError.couldNotAddScreen
    }

    if session.canAddOutput(output) {
      session.addOutput(output)
    } else {
      throw ScreenRecorderError.couldNotAddOutput
    }
